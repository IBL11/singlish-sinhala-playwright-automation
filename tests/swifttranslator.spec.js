...
test.describe("SwiftTranslator Singlish → Sinhala", () => {
  ...
  for (const tc of cases) {
    test(`${tc.id} - ${tc.name}`, async ({ page }) => {
      ...
      if (tc.type === "positive") {
        // Positive case: normal assertion
        expect(actual).toBe(normalize(tc.expected));
      } else {
        // Negative case: force a readable failure
        const fakeExpected = normalize(tc.expected) + "_FAIL"; // deliberately wrong
        expect(actual).toBe(fakeExpected); // will fail and show Expected vs Received
      }
